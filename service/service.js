...
exports.register = async function (
  ...
  if (userFound !== null) {
    ...
    let role = cat_sitter === 'on' || dog_sitter === 'on' || pet_walker === 'on' ? 'sitter' : 'owner';
...
exports.cancelBooking = async function (slotId) {
  try {
    return await cancelBookig(slotId);
  } catch (error) {
    console.error("Service could not book slot");
  }
};
...
exports.deleteSlot = async function (slotId) {
  try {
    return await deleteSlot(slotId);
  } catch (error) {
    console.error("Service could not book slot");
  }
};
...
exports.appendWithNextAvailableSlot = async (sitters) => {
  try {
    for (let i = 0; i < sitters.length; i++) {
      ...
      slots.dataValues.beg = slots.beginDateTime.toLocaleDateString("en-fr", {
        weekday: "long",
        hour: "numeric",
        minute: "numeric",
      });
      slots.dataValues.end = slots.endDateTime.toLocaleDateString("en-fr", {
        weekday: "long",
        hour: "numeric",
        minute: "numeric",
      });
...
exports.appendWithNextAvailableSlots = async (sitter) => {
  try {
    ...
    // format date time to human readable
    slots.forEach((slot) => {
      slot.dataValues.beg = slot.beginDateTime.toLocaleDateString("en-fr", {
        weekday: "long",
        hour: "numeric",
        minute: "numeric",
      });
      slot.dataValues.end = slot.endDateTime.toLocaleDateString("en-fr", {
        weekday: "long",
        hour: "numeric",
        minute: "numeric",
      });
...
exports.formatTime = async (slots) => {
  for (let i = 0; i < slots.length; i++) {
    slots[i].beg = slots[i].beginDateTime.toLocaleDateString("en-fr", {
      weekday: "long",
      hour: "numeric",
      minute: "numeric",
    });
    slots[i].end = slots[i].endDateTime.toLocaleDateString("en-fr", {
      weekday: "long",
      hour: "numeric",
      minute: "numeric",
    });
...
exports.getSittersInPostalCode = async (postalCode) => {
  try {
    let users = await findAllUsers();
    let sitters = users.filter(
      (user) => user.role === "sitter" && user.postalCode === postalCode
    );
    sitters = JSON.stringify(sitters);
    sitters = JSON.parse(sitters);
    return sitters;
...
exports.getSittersInCity = async (city) => {
  try {
    ...
    let sitters = users.filter(
      (user) => user.role === "sitter" && user.city === city
    );
    sitters = JSON.stringify(sitters);
    sitters = JSON.parse(sitters);
    return sitters;
